...
pub struct Category {
    ...
    pub is_featured: Option<bool>, // Can be null, defaults to false
    pub created_at: Option<DateTime<Utc>>, // Can be null, defaults to now()
    pub product_count: Option<i32>, // Can be null
}
```

**Field Descriptions:**

- `category_id`: Unique identifier (Primary Key, auto-generated)
- `category_name`: Category display name (required, non-empty)
- `is_featured`: Whether category should be prominently displayed (nullable,
  defaults to false)
- `created_at`: Category creation timestamp (nullable, defaults to now())
- `product_count`: Number of products in this category (nullable)
...
CREATE TABLE public.categories (
  ...
  constraint chk_category_name_not_empty check ((char_length(category_name) > 0)),
  constraint chk_product_count_non_negative check (
    (
      (product_count is null)
      or (product_count >= 0)
    )
  )
...
**Note:** `isFeatured`, `created_at`, and `product_count` can be `null` in the
database. The API will return the actual values from the database, including
