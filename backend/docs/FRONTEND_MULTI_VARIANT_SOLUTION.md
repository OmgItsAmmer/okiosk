...
5. ✅ **Queue Management Fixed**: Shared queue service between command executor
   and confirmation handler
6. ✅ **Comprehensive Logging**: Full queue operation tracing for debugging
...
#### Issue: "No active queue found"

**Symptoms:**

- Frontend sends confirmation request
- Backend responds with "No active queue found"

**Backend Logs to Check:**

```
[QUEUE] Creating queue 'session_123_0' with 1 items
[QUEUE_SERVICE] Creating queue 'session_123_0' with 1 actions
[QUEUE_SERVICE] Queue 'session_123_0' created successfully
[QUEUE] Confirmation request for queue 'session_123_0'
[QUEUE_SERVICE] Queue 'session_123_0' exists, locked: false
```

**Possible Causes:**

1. **Session ID Mismatch**: Frontend and backend using different session IDs
2. **Queue Not Created**: Initial command didn't trigger queue creation
3. **Queue Expired**: Queue timed out (5 minutes default)

**Solutions:**

1. Verify session_id is consistent between requests
2. Check that initial command returns `queue_info` in response
3. Ensure frontend calls `/api/ai/variant-confirm` quickly after variant
   selection

#### Issue: Queue Created But Not Found

**Check Backend Logs:**

- Look for `[QUEUE_SERVICE] Queue 'X' created successfully`
- Look for `[QUEUE_SERVICE] Queue 'X' not found` in confirmation

**Solution:**

- Ensure same queue service instance is used (fixed in latest version)

#### Issue: Frontend Shows Error Instead of Variants

**Check Response Structure:**

```json
// ✅ Correct response (has queue_info)
{
  "action_type": "variant_selection",
  "data": {
    "queue_info": {"position": 1, "total": 2}
  }
}

// ❌ Wrong response (no queue_info)
{
  "action_type": "variant_selection", 
  "data": {
    // Missing queue_info
...
- Update frontend to check for `queue_info` field
- Use `handleSequentialVariantSelection` when `queue_info` exists
