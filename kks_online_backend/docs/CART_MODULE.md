...
When using AI voice commands with the `/api/ai/command` endpoint, the
`action_type` field in the response follows this pattern:

- **Cart-related actions:** `action_type` contains the action name (e.g.,
  `"add_to_cart"`, `"remove_from_cart"`, `"update_quantity"`, `"clear_cart"`,
  `"view_cart"`, `"generate_bill"`, `"checkout"`)
- **Non-cart actions:** `action_type` is `null` (e.g., `search_product`,
  `show_menu`)

This allows the frontend to easily identify cart operations that require local
state management while treating other actions as informational responses.
